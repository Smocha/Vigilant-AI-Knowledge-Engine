# Vigilant Compliance Requirement Finder - hardened v6
# Adds normative requirement gating and broadens final evidence to 5 sources.
...
def is_normative_requirement(
    ...
    """Determine whether a claim is a requirement rather than just a topic.

    The generated claim can qualify through explicit normative language or an
    imperative-style action verb. Otherwise, the verified source evidence must
    itself contain normative language. This keeps phrases such as
    "Protecting Controlled Unclassified Information (CUI)" from being treated
    as a formal requirement when the source only describes the topic.
    """

    if contains_normative_modal(requirement):
        return True

    if starts_with_normative_action(requirement):
        return True
...
A formal requirement must be normative/actionable. Prefer explicit source
language such as must, shall, required to, requires, implement, restrict,
protect, limit, ensure, establish, maintain, or comparable obligation/action
language. Do not return a topical noun phrase such as "Protecting CUI" as a
formal requirement when the passage only describes the importance or scope
of that topic.
