...
HEADER_RE = re.compile(r"^(03\.\d{2}\.\d{2})[ \t]+(.+?)\s*$")
...
FAMILY_SECTION_RE = re.compile(r"^\s*3\.\d{1,2}\.\s+.+?\s*$")
...
def is_noise_line(line: str) -> bool:
    ...
    if collapsed.lower() == "may 2024":
        return True

    # pdftotext -layout may preserve large spacing between the two running-header
    # columns, so compare after whitespace normalization rather than with literal spaces.
    if "NIST SP 800-171r3" in collapsed and len(collapsed) <= 140:
        return True

    if collapsed.lower() == "protecting controlled unclassified information":
        return True
...
def parse_requirement_clauses(
    ...
    for idx in range(block_start + 1, block_end):
        ...
        # Stop at a family heading such as "3.4. Configuration Management".
        # This prevents a withdrawn/base requirement from absorbing the next family name.
        if FAMILY_SECTION_RE.match(raw):
            flush_clause()
            break

        # Defensive stop if another authoritative requirement header appears.
...
def parse() -> list[dict]:
    ...
    # Preserve physical newline-based line numbers exactly as tools such as grep -n report them.
    # str.splitlines() also treats form-feed characters (\f) as line boundaries, which
    # shifts provenance line numbers in pdftotext output. Split only on literal newlines.
    lines = SOURCE_TEXT.read_text(encoding="utf-8", errors="replace").split("\n")
    ...
    for header_index, (start_idx, requirement_id, title) in enumerate(headers):
        ...
        for clause in clauses:
            is_withdrawn = title.strip().lower() == "withdrawn"
            ...
            record = {
                ...
                "record_type": "withdrawn" if is_withdrawn else "requirement",
                "normative": not is_withdrawn,
                "family": {
                    "family_id": family_id,
...
def main() -> None:
    ...
    active_records = sum(1 for r in records if r["record_type"] == "requirement")
    withdrawn_records = sum(1 for r in records if r["record_type"] == "withdrawn")
    print(f"Active requirement records: {active_records}")
    print(f"Withdrawn records: {withdrawn_records}")
