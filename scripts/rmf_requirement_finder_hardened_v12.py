# Vigilant Compliance Requirement Finder - hardened v12
# Hardens deterministic extraction against legal/front-matter boilerplate and weak modal false positives.
...
META_EVIDENCE_PATTERNS = (
    ...
    # Legal/publication boilerplate is not a security control. These patterns
    # also give front matter a reranking penalty before deterministic extraction.
    r"\bpatent(?:s| infringement)?\b",
    r"\blicen[cs]e(?:s|d|ing)?\b",
    r"\bno representation is made or implied\b",
    r"\binformation technology laboratory\b",
    r"\buse may be required for compliance\b",
    r"\bguidance or requirements of this publication\b",
    r"\bpublication may be subject to\b",
)

NON_CONTROL_BOILERPLATE_PATTERNS = (
    r"\bpatent(?:s| infringement)?\b",
    r"\blicen[cs]e(?:s|d|ing)?\b",
    r"\bno representation is made or implied\b",
    r"\binformation technology laboratory\b",
...
def _is_complete_evidence_sentence(text):
    ...
    if str(text).rstrip().endswith((":", ";", ",", "(")):
        return False
    if re.search(r"\b(?:no|not|without|including|such as)\s*$", normalized):
        return False
    return True
...
def _looks_like_non_control_boilerplate(text):
    normalized = normalize_text(text or "")
    return any(
        re.search(pattern, normalized)
        for pattern in NON_CONTROL_BOILERPLATE_PATTERNS
    )


def _is_strong_normative_source_sentence(text):
    """Return True only for affirmative, control-like normative source text.

    Broad-query deterministic extraction must not confuse publication/legal
    boilerplate with a cybersecurity requirement. In particular, phrases such
    as "use may be required" and "licenses are not required" are not
    security controls even though they contain words used by the normative gate.
    """
    normalized = normalize_text(text or "")
    if not normalized:
        return False

    if _looks_like_meta_evidence(text) or _looks_like_non_control_boilerplate(text):
        return False

    # Negative or conditional statements about whether something is required
    # are not affirmative control requirements.
    if re.search(r"\b(?:is|are|was|were)\s+not\s+required\b", normalized):
        return False
    if re.search(r"\bnot\s+required\s+to\b", normalized):
        return False
    if re.search(r"\b(?:may|might|could)\s+be\s+required\b", normalized):
        return False

    action_text = _strip_requirement_prefix(text)
    action_normalized = normalize_text(action_text)

    # "Use" is a valid imperative in real controls ("Use replay-resistant...")
    # but is also commonly a noun in boilerplate ("Use may be required...").
    if re.match(r"^use\s+(?:may|might|could|can|is|are|was|were)\b", action_normalized):
        return False

    if starts_with_normative_action(action_text):
        return True

    # Strong affirmative modals from the actual source. Avoid the broader
    # legacy helper here because deterministic extraction is intentionally
    # stricter than ordinary claim validation.
    if re.search(r"\bmust(?:\s+not)?\b", normalized):
        return True
    if re.search(r"\bshall(?:\s+not)?\b", normalized):
        return True
    if re.search(r"\b(?:is|are)\s+required\s+to\b", normalized):
        return True
    if re.search(r"\brequires?\s+(?:the\s+)?(?:organization|system|component|user|administrator|agency|contractor)\b", normalized):
        return True
...
def build_deterministic_normative_requirements(question, sources, limit=MAX_DETERMINISTIC_REQUIREMENTS):
    ...
    for source in sources:
        ...
        for sentence in evidence_sentences(content):
            ...
            if not _is_strong_normative_source_sentence(sentence):
                continue
