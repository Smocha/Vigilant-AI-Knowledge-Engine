...
def normalize(text: str) -> str:
    text = text.replace("\u00ad", "")
    # pdftotext may leave a line-wrap space after a hyphen ("organization- defined")
    # while the parser rejoins it as "organization-defined". Treat those as equivalent
...
def is_source_noise_line(line: str) -> bool:
    """
    Identify PDF running headers/footers that the parser intentionally removes.

    Provenance should compare the structured requirement against the meaningful
    source text, not against page headers inserted in the middle of a wrapped clause.
    """
    stripped = line.strip()
    if not stripped:
        return True
...
def cleaned_source_slice(lines: list[str], line_start: int, line_end: int) -> str:
    """
    Reconstruct the authoritative source span using the same page-noise policy
    as the parser. This preserves the original provenance line range while
    ignoring headers/footers that occur inside that range.
    """
    meaningful = []
...
def main() -> None:
    ...
    for record in records:
        ...
        if record_type == "withdrawn":
            withdrawn_records.append(record)
            if normative is not False:
                errors.append(f"Withdrawn record marked normative: {record_id}")
        elif record_type == "requirement":
            active_records.append(record)
            if normative is not True:
                errors.append(f"Active requirement marked non-normative: {record_id}")
        else:
            errors.append(f"Unexpected record_type for {record_id}: {record_type!r}")
        ...
        source_slice = cleaned_source_slice(
    ...
    print(f"Active requirement records: {len(active_records)}")
    print(f"Withdrawn records: {len(withdrawn_records)}")
