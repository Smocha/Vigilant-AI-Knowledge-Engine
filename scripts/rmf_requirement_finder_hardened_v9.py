# Vigilant Compliance Requirement Finder - hardened v9
# Adds document-aware retrieval, fragment-safe fallbacks, and accurate validation counters.
...
def rerank_qdrant_points(points, question):
    ...
    for point in points:
        ...
        rerank_score = semantic_score + bonus - penalty
...
def search_qdrant(question):
    ...
    preferred_normalized = {name.strip().lower() for name in preferred_collections}
        ...
        try:
            ...
            for point in response.points:
                if (
                    ...
                    and point.score >= collection_min_score
                ):
                    payload = dict(
                        point.payload or {}
                    )
    ...
    preferred_points = [
        ...
        ).strip().lower() in preferred_normalized
    ]
    ...
    if preferred_collections and len(preferred_points) >= MAX_SOURCES:
        ...
        selected_ids = {id(point) for point in selected_points}
        for point in all_points:
    ...
    retrieval_stats = {
        ...
        "preferred_collections": [name.strip() for name in preferred_collections],
